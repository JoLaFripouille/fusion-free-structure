...
Si une nuance est absente, un acier générique réel des bibliothèques chargées est copié dans le document avec `Design.materials.addByCopy`. Les cinq propriétés indispensables à cette première étude linéaire sont identifiées par leurs identifiants internes ou leurs noms Fusion, puis écrites avec conversion par le gestionnaire d'unités du document. Les valeurs sont immédiatement relues. Toute création incomplète est supprimée avant d'être utilisée.
