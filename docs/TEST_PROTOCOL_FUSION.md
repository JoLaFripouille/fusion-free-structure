...
4. Arrêter puis exécuter le complément et vérifier `Jonctions acier V1.16.3` dans `STRUCTURE JHR > MODIFIER`.
...
2. Arrêter puis exécuter le complément et vérifier `Jonctions acier V1.16.3` dans `STRUCTURE JHR > MODIFIER`.
...
2. Ouvrir `Jonctions acier V1.16.3`, conserver `Jonction ajustée sur une barre de référence`, puis sélectionner la référence comme `Barre principale` et la barre à modifier comme `Barre secondaire`.
...
### 21G — Principale trop courte pour couvrir la secondaire

1. Créer une jonction ajustée exactement à l'extrémité d'une barre principale, avec un profil secondaire assez large pour dépasser cette extrémité.
2. Vérifier que le rapport affiche `Couverture principale : Insuffisante` avec une longueur positive.
3. Valider puis vérifier `PROLONGEMENT_BARRE_PRINCIPALE` dans le composant principal.
...
2. Arrêter puis exécuter le complément et vérifier `Grugeage profils ouverts V1.20.2` dans `STRUCTURE JHR > MODIFIER`.
...
10. Passer le `Jeu contre l'appui` de `1 mm` à `0 mm`, puis à `2 mm` : seul le rectangle orange doit se déplacer par rapport à l'âme principale.
...
14. Dans le composant secondaire, vérifier la présence de `PLAN_GRUGEAGE_EXTREMITE`, `PLAN_DEBUT_GRUGEAGE`, `PLAN_COUPE_AME_PRINCIPALE`, `PLAN_REFERENCE_ESQUISSE_GRUGEAGE`, `COUPE_DROITE_AME_PRINCIPALE`, `RETRAIT_APRES_AME_PRINCIPALE`, `ESQUISSE_OUTILS_GRUGEAGE` et `GRUGEAGE_PROFIL_OUVERT`.
...
2. Ouvrir `Grugeage profils ouverts V1.20.2`, sélectionner la principale puis la secondaire et vérifier `Angle entre axes : 60°`.
...
7. Cliquer sur `OK` puis contrôler `PLAN_STATION_PRINCIPALE_GRUGEAGE`, `PLAN_GRUGEAGE_EXTREMITE`, `AXE_ORIENTATION_COUPE_AME`, `PLAN_ORIENTATION_AME_PRINCIPALE`, `PLAN_DEBUT_GRUGEAGE`, `PLAN_COUPE_AME_PRINCIPALE` et `PLAN_REFERENCE_ESQUISSE_GRUGEAGE`.
...
## Test 24 — Grugeage cornières et tés

Valider une seule combinaison à la fois et annuler immédiatement en cas d'écart entre la preview et la coupe finale.

### 24A — Cornière vers cornière à 90°
...
3. Ouvrir `Grugeage profils ouverts V1.20.2`, sélectionner la principale puis la secondaire.
...
6. Modifier `Jeu sous l'âme secondaire` de `1 mm` à `0 mm`, puis `2 mm` : la limite droite rouge et la naissance de l'arrondi doivent se déplacer ensemble de la valeur exacte saisie, sans changer le rayon.
7. Modifier ensuite `Jeu autour du congé principal` de `1 mm` à `0 mm`, puis `2 mm` : seul le rayon rouge arrondi doit changer, sans déplacer la limite droite sous l'âme.
8. Vérifier que le plan orange se trouve sur la face de la branche verticale principale orientée vers la secondaire.
9. Vérifier le vert uniquement si la principale est trop courte pour couvrir la secondaire.
10. Cliquer sur `OK`, vérifier `GRUGEAGE_PROFIL_OUVERT`, puis `DEGAGEMENT_CONGE_PRINCIPAL` lorsque le rayon est nécessaire, un corps secondaire unique et la superposition de la coupe réelle avec la preview.
11. Mesurer le jeu sous l'âme secondaire : il doit correspondre exactement à la valeur saisie.
12. Contrôler que le dégagement arrondi échappe le congé principal sans retirer inutilement le congé de la secondaire.
13. Refaire une seule fois l'essai sur un IPE/HEA/HEB : les deux réglages cornière/té doivent disparaître ; il doit conserver ses deux volumes rouges et son ancien jeu vertical, sans créer `DEGAGEMENT_CONGE_PRINCIPAL`.
14. Annuler puis recommencer avec une cornière inégale avant de poursuivre.

### 24B — Les trois autres combinaisons

Après validation complète de 24A, effectuer séparément :

1. cornière principale → té secondaire ;
2. té principal → cornière secondaire ;
3. té principal → té secondaire.

Pour chaque essai, contrôler d'abord à `90°` le volume rouge unique, la face orange, le prolongement éventuel et le corps final. Ensuite seulement, reprendre un cas à `60°`, puis tester une rotation, un miroir et un ancrage non centré. Un second grugeage doit rester possible à l'extrémité opposée de la secondaire, tandis qu'un doublon sur la même extrémité doit être refusé.
...
2. Annuler complètement toute tentative créée avec une version précédente, puis arrêter et exécuter le complément et vérifier la version `V1.23.5`.
3. Ouvrir `STRUCTURE JHR > ASSEMBLAGES > Assemblage par cornières V1.23.5`.
